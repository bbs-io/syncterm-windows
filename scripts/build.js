...
const buildSetup = () => {
  ...
  const inno6global = path.normalize('C:/Program Files (x86)/Inno Setup 6/ISCC.exe');
  const inno6user = path.normalize(`${process.env.LOCALAPPDATA}/Programs/Inno Setup 6/ISCC.exe`);
  const iscc = fs.existsSync(inno6global) ? inno6global : fs.existsSync(inno6user) ? inno6user : `ISCC.exe`;
